...
AppDataPreferences.prototype.lastMd5_ = null;
...
AppDataPreferences.prototype.sync = function (opt_callback) {
  ...
  this.getDriveService_().get(this.local, function(err, file) {
    ...
    var newMd5 = file['md5Checksum'];
    if (!that.lastMd5_) {
      // this is the initial sync, there's no difference
      // between remote and local.
      that.lastMd5_ = newMd5;
      return opt_callback && opt_callback();
    }

    if (newMd5 != that.lastMd5_) {
...
AppDataPreferences.prototype.merge_ = function(remote, opt_callback) {
  console.log('merging', this.local, 'with', remote);
  ...
  for (var key in this.remote) {
    this.local[key] = this.remote[key];
  }
  opt_callback && opt_callback();
};
